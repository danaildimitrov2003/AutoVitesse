...
extension SearchBarTextField {
    func handleSearch(_ search: String) {
        ...
        if search.count >= 2 {
            ...
            guard let url = URL(string: "https://car-data.p.rapidapi.com/cars?limit=50&page=0&model=\(modifiedSearch)")
            else {
                print("Invalid URL")
                return
            }
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let cars = try JSONDecoder().decode([АpiCar].self, from: data)
                    saveCars(cars)
                } catch {
                    print(error)
                }
    ...
    func saveCars(_ cars: [АpiCar]) {
        do {
            let realm = try Realm(configuration: config)
            realm.safeWrite {
                for apiCar in cars where realm.objects(Car.self).filter("apiId = %@", apiCar.id).first == nil {
                    let car = Car()
